...
class FileManager {
  ...
  async function sendData(webhookData, onServerDone = () => { }) {
    if (isBusySending) {
      waitingQueue.push([webhookData, onServerDone]);
    ...
    let promise = new Promise((resolve, reject) => {
      ...
      function onResponse(response) {
        ...
        if (waitingQueue.length > 0) {
          console.log(waitingQueue.length + " still left in waitingQueue. Sending another in 2 seconds.");
          setTimeout(() => {
            console.log("Sending...")
            let newArguments = waitingQueue.shift();
